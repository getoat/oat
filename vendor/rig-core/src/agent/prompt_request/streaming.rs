                ...
                while let Some(content) = stream.next().await {
                    match content {
                        ...
                        Ok(StreamedAssistantContent::Final(final_resp)) => {
                            ...
                            // Completion-finish hooks are response-scoped, not text-scoped.
                            // Tool-only turns still consume tokens and should be accounted for
                            // immediately so callers can persist accurate step-level usage even if
                            // a later turn fails before the overall multi-turn exchange completes.
                            if let Some(ref hook) = self.hook &&
